...
def test_tracking_identity_mapping():
    identity = make_tracking_identity("Hộp chia sữa", 1, day=date(2026, 5, 16))
    assert identity.sub1 == "facebook"
    assert identity.sub2 == "nangniutraingottinhyeu"
...
def test_utm_mapping():
    ...
    assert utm["utm_campaign"] == "affilipilot_mom_baby_202605"
