...
def evaluate_product_content(product: dict[str, Any], text: str) -> ProductContentResult:
    ...
    if category in {"electronics", "phone", "smartphone"} or any(term in title for term in ("điện thoại", "iphone", "samsung", "galaxy", "xiaomi")):
        if not any(term in lower for term in ("camera", "ảnh", "video", "pin", "bộ nhớ", "bảo hành", "trả góp")):
            reasons.append("missing_electronics_purchase_rationale")
            recommendations.append("Mention family camera, photos/videos, battery, storage, warranty, or installment rationale.")
            score -= 35
