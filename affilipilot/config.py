...
def load_config(secret_path: str | Path = DEFAULT_SECRET_PATH) -> AffiliPilotConfig:
    ...
    return AffiliPilotConfig(
        ...
        accesstrade_campaign_present=present("ACCESSTRADE_SHOPEE_CAMPAIGN_ID"),
...
def render_config_status(config: AffiliPilotConfig) -> str:
    return "\n".join([
        ...
        f"Accesstrade Shopee campaign: {'present' if config.accesstrade_campaign_present else 'pending/missing'}",
